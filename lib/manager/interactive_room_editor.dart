...
class _InteractiveRoomEditorState extends State<InteractiveRoomEditor> {
  ...
  Map<String,List<String>> _featuresByType = {
    ...
    'Conference/Professional Event': ['Stage','Writing Table','Screen','Charging Point'],
  };
  ...
  /// משכפל את כל האלמנטים שנבחרו,
  /// ומזיז את הכפילויות  (dx,dy) כלפי מטה־ימינה כדי שלא יתחברו לאלו המקוריים.
  Future<void> _duplicateSelection() async {
    const double offsetX = 60.0; // הזחה אופקית
    const double offsetY = 60.0; // הזחה אנכית
    ...
    // מצא את כל האלמנטים שנבחרו
    final toDuplicate = elements.where((e) => _selectedIds.contains(e['id'])).toList();
  ...
  Widget _buildOptionsSheet() {
    final allFeatures = _featuresByType.values.expand((l) => l).toSet().toList();
    ...
    return Positioned(
      ...
      child: GestureDetector(
        onTap: e['type'] == 'chair'
            ...
            ? () => _editTableShape(e)
            : null,
        onLongPress: () => _removeElement(e['id']),
        child: Draggable<Map<String, dynamic>>(
          data: e,
          childWhenDragging: Container(),
          onDragEnd: (details) async {
            // אם לא סלקציה, מזיז רק את זה
            ...
            // אם יש סלקציה, מזיז את כל הסלקציה ביחד
            final box = context.findRenderObject() as RenderBox;
            final local = box.globalToLocal(details.offset);
            // 1. חישוב מרכז האובייקט שיגררת
            final w = (e['w'] ?? 40.0) as double;
            final h = (e['h'] ?? 40.0) as double;

            // 2. ניישר כך שהמרכז של האלמנט יהיה בנקודת הגרירה
            ...
            setState(() {
              for (var elm in elements) {
                if (_selectedIds.contains(elm['id'])) {
                  ...
                  elm['x'] = newX;
                  elm['y'] = newY;
                  batch.update(col.doc(elm['id']), {'x': newX, 'y': newY});
                }
              }
            });
          ...
          child: _elementIcon(e, selected),
          feedback: _elementIcon(e, selected),
  ...
  Future<void> _editChairFeatures(Map<String,dynamic> chair) async {
    ...
    // קח את ההגדרות הקיימות:
    final selected = Set<String>.from((chair['features'] ?? []) as List<dynamic>);
    final result = await showDialog<Set<String>>(
      ...
      builder: (_) => AlertDialog(
            ...
            child: ListView(
              ...
              children: opts.map((feat) {
                return CheckboxListTile(
                  ...
                  onChanged: (v) {
                    setStateDialog(() {
                      if (v == true) selected.add(feat);
                      else selected.remove(feat);
                    });
                  },
                );
              }).toList(),
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () => Navigator.pop(context, selected), child: Text('שמור')),
        ],
      ),
    );
    ...
    if (result != null){
      await FirebaseFirestore.instance
          .collection('events').doc(widget.eventId)
          .collection('elements').doc(chair['id'])
...
// עזר להזזת פריט יחיד
    ...
    return BoxDecoration(
      image: DecorationImage(
        ...
        colorFilter: ColorFilter.mode(
          Colors.white.withOpacity(0.6), // שקיפות קלה לריכוך
          BlendMode.dstATop,
        ),
      ),
    );
      ...
      body: GestureDetector(
        ...
        onPanStart: (d) {
          setState(() {
            ...
            _selectionRect = Rect.fromLTWH(d.localPosition.dx, d.localPosition.dy, 0, 0);
          });
        },
        onPanUpdate: (d) {
          setState(() {
        ...
        onPanEnd: (_) {
          // כשמסיימים גרירה – בודקים מי בתוך ה־rect
          final sel = <String>{};
  ...
  /// מיישר את הבחירה בשורה אחת, עם מרווח שווה בין הפריטים
  /// מיישר את כל הכיסאות הנבחרים בשורה אחת, עם רווח קבוע של 50px ביניהם
  Future<void> _alignSelectionWithSpacing() async {
    const double spacing = 50.0;

    // בחר רק כיסאות מתוך הנבחרים
    final chairs = elements
        .where((e) =>
    _selectedIds.contains(e['id']) && e['type'] == 'chair')
        .toList();
    if (chairs.isEmpty) return;

    // מיון לפי X כדי לשמור על הסדר
    chairs.sort((a, b) =>
    ((a['x'] as num).compareTo(b['x'] as num)));

    // נקודת התחלה: ה־X של הכיסא הראשון, ו־Y קבוע (כל הכיסאות באותה שורה)
    ...
    setState(() {
      // תעדכן את ה־UI לפי הערכים החדשים
    });
  }


  /// יוצר קבוצת אלמנט חדשה ושומר את רשימת ה־IDs שלה
  Future<void> _groupSelection() async {
    ...
    final groupElement = {
      ...
      'id': docRef.id,
      // בשביל פשטות ניתן לאחסן מרכז גרפי:
      'x': 0.0,
      'y': 0.0,
    };
    // מוחקים את הפריטים המאוגדים
    final batch = FirebaseFirestore.instance.batch();
    final col = FirebaseFirestore.instance
        .collection('events').doc(widget.eventId)
        .collection('elements');
  ...
  /// סובב את כל הבחירה ב־90° סביב המרכז הממוצע שלה
    ...
    setState(() {
      for (var e in elements) {
        if (_selectedIds.contains(e['id'])) {
          ...
          final rotation = ((e['rotation'] as num?)?.toDouble() ?? 0.0) + (90 * 3.14159 / 180);
  ...
  Widget _elementIcon(Map<String, dynamic> e, bool selected) {
    final type = e['type'];
  ...
  Future<void> _alignSelectionToTable() async {
    // מוצאים את השולחן הראשון (או שולחן שנבחר) כדי לקבל את y שלו
    final table = elements.firstWhere((e) => e['type']=='table', orElse: () => {});
    if (table.isEmpty) return;

    final double tableY = table['y'];
    final double tableHeight = 32 + 16*2; // גובה האייקון + padding
    ...
    setState(() {
      for (var e in elements) {
        if (_selectedIds.contains(e['id']) && e['type']=='chair') {
          final newY = tableY + tableHeight + 10; // 10px מתחת לשולחן
          e['y'] = newY;
...
IconData _featureIconFor(String label) {
  switch (label.toLowerCase()) {
    case 'board': return Icons.border_color;
    ...
    case 'dance floor': return Icons.directions_run;
    case 'speakers': return Icons.surround_sound;
    case 'stage': return Icons.theater_comedy;
    case 'writing table': return Icons.edit;
    case 'screen': return Icons.tv;
    case 'charging point': return Icons.battery_charging_full;
